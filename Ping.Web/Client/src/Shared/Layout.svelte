<script>
	...
	const menu_links = [
		...
		{ name: 'Manage Users', url: '/users', role: 'owner' },
...
<div class="min-h-full">
	<nav class="bg-gray-800 py-2">
			...
			<div class="flex h-16 items-center justify-between">
							...
							<MenuItems
								...
								{#each menu_links as link}
									...
									{#if !link.role || link?.role === auth?.role}
										<MenuItem let:active>
											{@const classes = `${active ? 'bg-gray' : ''} block text-left w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100`}
											{#if url === '/logout'}
												<button class={classes} use:inertia={{ href: url, method: 'post' }} role="menuitem" tabindex="-1">{link.name}</button>
											{:else }
												<a
													href={url}
													use:inertia
				...
				<div class="-mr-2 flex md:hidden">
					...
					<button
						...
						on:click={handle_menu_button_click}
		...
		<div class="md:hidden" class:hidden={!show_menu} id="mobile-menu">
				...
				<div class="mt-3 space-y-1 px-2">
					{#each menu_links as link}
						{@const active =
							...
							($page.url.startsWith(link.url) && link.url.length > 1)}
                        {@const url = link.url === '/profile' ? `/users/${auth?.userId}/edit` : link.url}
						{@const classes = `block rounded-md px-3 py-2 text-base font-medium ${
								active
								? 'bg-gray-900 text-white'
								: 'text-gray-300 hover:bg-gray-700 hover:text-white'
						}`}
						{#if !link.role || link?.role === auth?.role}
							{#if link.url === '/logout'}
								<button use:inertia={{ href: url, method: 'post' }} class={classes}>{link.name}</button>
							{:else}
								<a
									use:inertia
									href={url}
